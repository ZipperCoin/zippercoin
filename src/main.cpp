...
uint256 hashGenesisBlock("0x789a5d77af084890f47268a855c822413075a230b5a4e3c3ca2dddc2b502696a");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // ZipperCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "ZipperCoin Signed Message:\n";
...
{
    // ZipperCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    if(nHeight == 1){
        nSubsidy = nSubsidy * 70000000 * 70  / 100 / 50;
     }
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // ZipperCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // ZipperCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // ZipperCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x789a5d77af084890f47268a855c822413075a230b5a4e3c3ca2dddc2b502696a");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1400915934;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085974038;
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ZipperCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ZipperCoinMiner : ProcessBlock, block not accepted");
    }
...
void static LitecoinMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        printf("Running ZipperCoin with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ZipperCoinMiner terminated\n");
        throw;
    }
}
